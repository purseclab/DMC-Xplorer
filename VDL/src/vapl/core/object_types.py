"""Implementations of the built-in vapl classes."""

import collections
import math
import random

from core.distributions import Samplable, needsSampling
from core.specifiers import Specifier, PropertyDefault
from core.vectors import Vector
from core.geometry import (_RotatedRectangle, averageVectors, hypot, min,
                                  pointIsInCone)
from core.regions import CircularRegion, SectorRegion
from core.type_support import toVector, toHeading, toType
from core.lazy_eval import needsLazyEvaluation
from core.utils import DefaultIdentityDict, areEquivalent, cached_property
from core.errors import RuntimeParseError

## Abstract base class

class _Constructible(Samplable):
	"""Abstract base class for vapl objects.

	vapl objects, which are constructed using specifiers, are implemented
...
class Point(_Constructible):
	"""Implementation of the vapl base class ``Point``.
	...
	.. note::
		...
		If you're looking into vapl's internals, note that `Point` is actually a
...
class OrientedPoint(Point):
	"""Implementation of the vapl class ``OrientedPoint``.
...
class Object(OrientedPoint, _RotatedRectangle):
	"""Implementation of the vapl class ``Object``.

	This is the default base class for vapl classes.
	...
	def _specify(self, prop, value):
		# Normalize types of some built-in properties
		if prop == 'behavior':
			import syntax.veneer as veneer	# TODO improve?
	...
	def show(self, workspace, plt, highlight=False):
		...
		pos = self.position
		spos = workspace.vaplToSchematicCoords(pos)
		...
		if highlight:
			...
			camera = self.position.offsetRotated(self.heading, self.cameraOffset)
			cpos = workspace.vaplToSchematicCoords(camera)
			for angle in (-ha, ha):
				p = camera.offsetRadially(20, self.heading + angle)
				edge = [cpos, workspace.vaplToSchematicCoords(p)]
				x, y = zip(*edge)
				plt.plot(x, y, 'b:')

		corners = [workspace.vaplToSchematicCoords(corner) for corner in self.corners]
		x, y = zip(*corners)
