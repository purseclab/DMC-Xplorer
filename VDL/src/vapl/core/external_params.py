"""Support for values which are sampled outside of vapl.

External Samplers in General
============================

External samplers provide a mechanism to use different types of sampling
techniques, like optimization or quasi-random sampling, from within a vapl
program. Ordinary random values in vapl are instances of `Distribution`;
this module defines a special subclass, `ExternalParameter`, representing a
value which is sampled externally. vapl programs with external parameters
...
samplers in the `VerifAI`_ toolkit (which in turn can use vapl as a modeling
language).
...
This is equivalent to the ordinary vapl line ``ego = Object at (5, 15) @ 0``,
...
vapl provides a convenient way to define this prior using the ordinary syntax
for distributions::
...
from core.distributions import Distribution, Options
...
class ExternalSampler:
	...
	Attributes:
		rejectionFeedback: Value passed to the `sample` method when the last sample was rejected.
		  This value can be chosen by a vapl scenario using the global parameter
...
class VerifaiSampler(ExternalSampler):
	...
	The sampler can be configured using the following vapl global parameters:
...
class ExternalParameter(Distribution):
	"""A value determined by external code rather than vapl's internal sampler."""
	def __init__(self):
		super().__init__()
		self.sampler = None
...
class VerifaiRange(VerifaiParameter):
	"""A :obj:`~core.distributions.Range` (real interval) sampled by VerifAI."""
...
class VerifaiDiscreteRange(VerifaiParameter):
	"""A :obj:`~core.distributions.DiscreteRange` (integer interval) sampled by VerifAI."""
...
class VerifaiOptions(Options):
	"""An :obj:`~core.distributions.Options` (discrete set) sampled by VerifAI."""
