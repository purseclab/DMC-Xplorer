...
from core.distributions import needsSampling
from core.regions import Region, everywhere
from core.geometry import findMinMax
...
class Workspace(Region):
	...
	def zoomAround(self, plt, objects, expansion=1):
		...
		if not objects:
			return
		positions = (self.vaplToSchematicCoords(obj.position) for obj in objects)
		x, y = zip(*positions)
	...
	def vaplToSchematicCoords(self, coords):
		"""Convert vapl coordinates to those used for schematic rendering."""
		return coords
