...
#dict [{<vapl.classes.light object at 0x105B2430>: [{'dim': 10}, 'brighten', 'switchOff'],
    ...
    def generate(self, commandRules):
     ...
     for i in range(0,len(VaCommands)):
      ...
      #environment='{"VA-Name":VaCommands[i].Va.name,"VA-Type":1,"VA-Location":1,"VA-ID":1,"deviceName":1,"deviceType":1,"deviceLocation":1,"DeviceID":1,"DeviceID":1,"DeviceConnectedVA":1}'
    ...
    def addWakeup(self,command,vaType):
        if(vaType.lower()=="alexa"):
            ...
            st='Hey Google, '+ st
            return st
    def getSkillList(typename, objects=None):
        if objects is None:
            objects = list(gc.get_objects())
        skillList=[]
        print(type(objects), "here please")
        for o in objects:
            if(type(o)==typename):
                skillList.append(o)
        return skillList
    ...
    def convertToJson(self, VaCommands):
       ...
       vaList=[]
       deviceList=[]
       skillJson={"skills":None}
       tempDictSkill={}
       skillList=alexaSkill.instances

       #skillList.append(self.getSkillList(googleSkill))
       for i in range(0,len(skillList)):

           tempskillJson={"skillName":skillList[i].name,"skillToken":skillList[i].token,"clientSecret":skillList[i].clientSecret,"clientID":skillList[i].clientID,"action":skillList[i].action,"device":skillList[i].device}
           if(type(skillList[i])==alexaSkill):
            tempskillJson["skillType"]="Alexa"
           elif(type(skillList[i])==googleSkill):
               tempskillJson["skillType"]="Google"
           y11=json.dumps(tempskillJson)
           y11 =json.loads(y11)

           tempDictSkill[y11["skillName"]]=y11
           print(tempDictSkill,"dictdevice")
       skillJson["skills"]=tempDictSkill
       print(skillJson,"json111")
       ...
       print(vaJson,"json")
       print(deviceList)
       deviceJson={"SmartHomeDevice":None}
       tempDictDevice={}
       for i in range(0,len(deviceList)):

        tempDeviceJson={"deviceName":deviceList[i].name,"deviceType":type(deviceList[i]).__name__,"deviceLocation":deviceList[i].location,"deviceManufacturer":deviceList[i].manufacturer,"deviceID":deviceList[i].id,"deviceConnectedVA":deviceList[i].connectedVA.id}
        y1=json.dumps(tempDeviceJson)
        y1 =json.loads(y1)

        tempDictDevice[y1["deviceName"]]=y1
        print(tempDictDevice,"dictdevice")
       deviceJson["SmartHomeDevice"]=tempDictDevice
       print(deviceJson,"json1")

       vcJson={"VoiceCommand":None}
       tempDictVC={}
       for i in range(0,len(VaCommands)):
           tempVCJson={"command":VaCommands[i].voiceCommand,"timestamp":VaCommands[i].timestamp,"deviceID":VaCommands[i].device.id,"vaID":VaCommands[i].Va.id}
           y2=json.dumps(tempVCJson)
           y2 =json.loads(y2)
           tempDictVC[i]=y2
       vcJson["VoiceCommand"]=tempDictVC
       print(vcJson,"json2")
       ...
       json_object_skills = json.dumps(skillJson,indent = 4)
       ...
       with open("skills.json", "w") as outfile:
           outfile.write(json_object_skills)
       #print(y1,"json1")
...
class alexaSkill():
    instances = []
    def __init__(self, name,token, clientSecret, clientID, action, device):
        self.name=name
        self.token=token
...
"""
TODO when google skill attributes are finilized
class googleSkill():
    def __init__(self, token, clientSecret, clientID, action, device):
        self.token=token
        self.clientSecret=clientSecret
        self.clientID=clientID
        self.action=action
...
class action:
    def __init__(self, device, name, paramters, commandStructure):
        self.device=device
        self.name=name
...
classList=[light,thermostat,television,smartLock,fan,airPurifier,smartCamera,vacuumCleaner,microwave,refrigerators,washingMachine]
