...
from core.distributions import Samplable, needsSampling, toDistribution
from core.lazy_eval import needsLazyEvaluation
from core.object_types import _Constructible
import core.errors as errors
from core.errors import (TokenParseError, PythonParseError, ASTParseError,
								InvalidScenarioError)
import core.dynamics as dynamics
import core.pruning as pruning
import syntax.veneer as veneer

### THE TOP LEVEL: compiling a vapl program

def scenarioFromString(string, params={}, model=None, scenario=None,
					   filename='<string>', cacheImports=False):
	"""Compile a string of vapl code into a `Scenario`.
...
def scenarioFromFile(path, params={}, model=None, scenario=None, cacheImports=False):
	...
	if not extension or extension[1:] not in vaplExtensions:
		ok = ', '.join(vaplExtensions)
		err = f'vapl scenario does not have valid extension ({ok})'
	...
	try:
		...
		if not cacheImports:
			...
			for name, module in sys.modules.items():
				if name not in oldModules and getattr(module, '_isvaplModule', False):
...
def compileStream(stream, namespace, params={}, model=None, filename='<stream>'):
	...
	if verbosity >= 2:
		veneer.verbosePrint(f'  Compiling vapl module from {filename}...')
		startTime = time.time()
	...
	# pull in new constructor (vapl class) definitions, which change the way
	...
	if verbosity >= 2:
		...
		veneer.verbosePrint(f'  Compiled vapl module in {totalTime:.4g} seconds.')
...
namespaceReference = '_vapl_module_namespace'		# used in the implementation of 'model'
...
statementRaiseMarker = '_vapl_statement_'

## Try-interrupt blocks

interruptWhenStatement = ('interrupt', 'when')

interruptExceptMarker = '_vapl_interrupt_'

## Constructors and specifiers

# statement defining a new constructor (vapl class);
...
builtinConstructors = {
	...
	'sample':Constructor('sample',None),
	'va':Constructor('va',None),
	'light':Constructor('light',None),
	'thermostat':Constructor('thermostat',None),
	'television':Constructor('television',None),
	'smartLock':Constructor('smartLock',None),
	'fan':Constructor('fan',None),
	'airPurifier':Constructor('airPurifier',None),
	'smartCamera':Constructor('smartCamera',None),
	'vacuumCleaner':Constructor('vacuumCleaner',None),
	'microwave':Constructor('microwave',None),
	'refrigerators':Constructor('refrigerators',None),
	'washingMachine':Constructor('washingMachine',None),
	'timestamp':Constructor('timestamp',None),
	'probDist':Constructor('probDist',None),
	'alexaSkill':Constructor('alexaSkill',None),
...
scenarioMarker = '_vapl_scenario_'	# not a statement, but a marker for the parser
...
for op in infixOperators:
	# if necessary, set up map from vapl to Python syntax
	...
	# if necessary, set up map from Python to vapl semantics
...
## Meta path finder and loader for vapl files

vaplExtensions = ('vcl')

class vaplMetaFinder(importlib.abc.MetaPathFinder):
	def find_spec(self, name, paths, target=None):
		...
		for path in paths:
			for extension in vaplExtensions:
				filename = modname + '.' + extension
				filepath = os.path.join(path, filename)
				if os.path.exists(filepath):
					filepath = os.path.abspath(filepath)
					spec = importlib.util.spec_from_file_location(name, filepath,
						loader=vaplLoader(filepath, filename))
					return spec
		return None

class vaplLoader(importlib.abc.InspectLoader):
	...
	def exec_module(self, module):
		...
		# Mark as a vapl module
		module._isvaplModule = True
		# Save code, source, and translated source for later inspection
		module._code = code
		module._source = source
		module._pythonSource = pythonSource

		# If we're in the process of compiling another vapl module, inherit
	...
	def get_code(self, fullname):
		...
		assert module._isvaplModule, module
		return module._code
...
sys.meta_path.insert(0, vaplMetaFinder())
...
def findConstructorsIn(namespace):
	"""Find all constructors (vapl classes) defined in a namespace."""
	constructors = []
	print("##################################### Namespace ###################################")
	print(namespace)
...
class TokenTranslator:
	"""Translates a vapl token stream into valid Python syntax.

	This is a stateful process because constructor (vapl class) definitions
	...
	def translate(self, tokens):
		...
		for token in tokens:
			...
			# Catch Python operators that can't be used in vapl
				...
				if nextToken is not None:
					...
					if startOfLine and tstring == 'for':	# TODO improve hack?
						...
						if peek(tokens).exact_type == LPAR:		# superclass specification
							...
							while nextToken.exact_type != RPAR:
								...
								if base in self.constructors:
									vaplParents.append(base)
								if peek(tokens).exact_type == COMMA:
									next(tokens)
								nextToken = next(tokens)
							if not vaplParents and tstring != 'class':
								self.parseError(nextToken,
									f'vapl class definition with no vapl superclasses')
						if peek(tokens).exact_type != COLON:
							self.parseError(peek(tokens), 'malformed class definition')
						if not bases:
							bases = vaplParents = ('Object',)		# default superclass
						if vaplParents:
							self.createConstructor(nextString, vaplParents)
						...
						self.parseError(token,
							f'Python construct "{construct}" not allowed in vapl')
				if not matched:
					...
					if oneWord in allowedPrefixOps:		# 1-word prefix operator
						...
						self.parseError(token,
							f'Python construct "{tstring}" not allowed in vapl')
					else:
...
temporaryName = '_vapl_temporary_name'
behaviorArgName = '_vapl_current_behavior'
checkInvariantsName = '_vapl_check_invariants'
interruptPrefix = '_vapl_interrupt'
...
class ASTSurgeon(NodeTransformer):
	...
	def visit_BinOp(self, node):
		"""Convert infix operators to calls to the corresponding vapl internal functions."""
		left = node.left
		right = node.right
		op = node.op
	...
	def visit_Raise(self, node):
		"""Handle vapl statements encoded as raise statements.
	...
	def visit_Call(self, node):
		"""Handle vapl syntax and semantics of function calls.
	...
	def visit_AsyncFunctionDef(self, node):
		"""Process vapl constructs parsed as async function definitions.
	...
	def visit_ClassDef(self, node):
		"""Handle vapl constructs parsed as class definitions.

		In particular:
		  * transform vapl class definitions into ordinary Python ones;
		  * leave ordinary Python class definitions alone.
		"""
		if node.name in self.constructors:		# vapl class definition
			return self.transformvaplClass(node)
			...
			for base in node.bases:
				...
				if name is not None and name in self.constructors:
					self.parseError(node,
									f'Python class {node.name} derives from vapl class {name}')
			return self.generic_visit(node)

	def transformvaplClass(self, node):
		"""Process property defaults for vapl classes."""
		newBody = []
...
def translateParseTree(tree, constructors, filename):
	"""Modify the Python AST to produce the desired vapl semantics."""
...
def storeScenarioStateIn(namespace, requirementSyntax):
	"""Post-process an executed vapl module, extracting state from the veneer."""
	...
	def registerNamespace(modName, ns):
		...
		for name, value in ns.items():
			if (isinstance(value, types.ModuleType)
				and getattr(value, '_isvaplModule', False)):
...
def constructScenarioFrom(namespace, scenarioName=None):
	"""Build a Scenario object from an executed vapl module."""
