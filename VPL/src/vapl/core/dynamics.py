...
from core.distributions import Samplable, toDistribution, needsSampling
from core.errors import RuntimeParseError, InvalidScenarioError
from core.lazy_eval import needsLazyEvaluation
from core.requirements import (RequirementType, PendingRequirement,
                                      DynamicRequirement)
from core.simulators import (RejectSimulationException, EndSimulationAction,
                                    EndScenarioAction)
from core.utils import argsToString
...
class DynamicScenario(Invocable):
    ...
    def _requiresArguments(cls):
        ...
        try:
            sig.bind(None, None)    # first two arguments are added internally by vapl
            return False
        except TypeError:
            return True
        ...
        with veneer.executeInScenario(self):
            ...
            if self._compose is not None:
                if not inspect.isgeneratorfunction(self._compose):
                    from syntax.translator import composeBlock
        ...
        if self._ego is None and self._compose is None:
            ...
            if self._dummyNamespace and len(modScenarios) == 1:
                if modScenarios[0]._requiresArguments():
                    msg += ('\n(Note: this vapl file contains a modular scenario, but it\n'
                    ...
                    msg += ('\n(Note: this vapl file contains a modular scenario, but also\n'
